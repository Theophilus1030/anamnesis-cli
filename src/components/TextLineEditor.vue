    ...
    <div class="bg-zinc-900 rounded-lg p-6 w-250 max-h-[90vh] overflow-y-auto flex gap-6">
      ...
      <div class="flex-1">
        ...
        <div v-if="selectedGlyph" class="mb-4 flex items-center justify-center gap-2">
          ...
          />
          <button
            class="px-3 py-1 bg-blue-600 hover:bg-blue-500 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            :disabled="!replaceChar"
            @click="replaceCharacter"
          >
            替换
          </button>
        </div>
        ...
        <div v-if="selectedGlyph" class="mb-4 flex items-center justify-center gap-2">
          <button
            class="px-3 py-1 bg-orange-600 hover:bg-orange-500 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div class="mb-4 p-3 bg-zinc-800 rounded-lg">
          <div class="text-zinc-500 text-xs mb-2">点击字符进行选择和编辑:</div>
          <div class="flex flex-wrap gap-1">
            <button
              v-for="(glyph, index) in editingLine.glyphs"
              :key="glyph.id"
              :class="[
      ...
      <div class="w-75 border-l border-zinc-700 pl-6">
        ...
        <div v-if="sentence" class="space-y-4">
          ...
          <div>
            ...
            <div class="flex flex-col gap-2">
              <label class="flex items-center gap-2 cursor-pointer">
                <input 
                  type="radio" 
                  v-model="sentenceForm.type" 
                  value="text"
                  class="w-4 h-4 accent-blue-500"
                />
                <span class="text-zinc-300 text-sm">正文</span>
              </label>
              <label class="flex items-center gap-2 cursor-pointer">
                <input 
                  type="radio" 
                ...
                />
                <span class="text-zinc-300 text-sm">标题</span>
              </label>
              <label class="flex items-center gap-2 cursor-pointer">
                <input 
                  type="radio" 
                ...
                />
                <span class="text-zinc-300 text-sm">注释</span>
              </label>
            </div>
          </div>
          ...
          <!-- 正则原文 -->
          <div>
            <label class="block text-sm text-zinc-400 mb-1">正则原文</label>
            <textarea
              v-model="sentenceForm.normalized"
              class="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded text-zinc-100 text-sm focus:outline-none focus:border-blue-500 resize-none"
              rows="2"
          ...
          <!-- 英文翻译 -->
          <div>
            <label class="block text-sm text-zinc-400 mb-1">英文翻译</label>
            <textarea
              v-model="sentenceForm.en"
              class="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded text-zinc-100 text-sm focus:outline-none focus:border-blue-500 resize-none"
              rows="2"
              placeholder="English..."
            />
          ...
          <!-- 中文翻译 -->
          <div>
            <label class="block text-sm text-zinc-400 mb-1">中文翻译</label>
            <textarea
              v-model="sentenceForm.zh"
              class="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded text-zinc-100 text-sm focus:outline-none focus:border-blue-500 resize-none"
              rows="2"
              placeholder="中文..."
            />
