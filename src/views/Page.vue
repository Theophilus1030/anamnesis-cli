...
const startPanPos = ref({ x: 0, y: 0 }) // 记录鼠标按下时的屏幕坐标
const startTranslate = ref({ x: 0, y: 0 }) // 记录按下时的偏移量


// OCR 弹窗状态
const showOcrModal = ref(false)
const ocrIsProcessing = ref(false)
const ocrErrorMsg = ref('')

// OCR 模型配置
const ocrModelPath = ref('')
const ocrSegModelPath = ref('')
const useDefaultRecModel = ref(true)
const useDefaultSegModel = ref(true)
...
// --- 添加检查默认模型的函数 ---
async function checkDefaultModels() {
  try {
...
function openOcrModal() {
  // 1. 检查是否有图片
  if (!meta.value?.image_remote_url && !meta.value?.image_local_path) {
    alert('请先在“元信息”中设置图片路径 (远程URL或本地路径)')
    return
  }
...
// --- 选择模型文件 ---
async function selectOcrModel() {
  const file = await open({
    filters: [{ name: 'Model', extensions: ['mlmodel'] }]
  })
  if (file) {
    ocrModelPath.value = file as string
    useDefaultRecModel.value = false // 自动取消勾选默认
  }
}

async function selectOcrSegModel() {
  const file = await open({
    filters: [{ name: 'Model', extensions: ['mlmodel'] }]
  })
  if (file) {
    ocrSegModelPath.value = file as string
    useDefaultSegModel.value = false // 自动取消勾选默认
  }
}

// --- 执行 OCR ---
async function runOcr() {
  // 获取图片路径：优先使用本地路径，其次使用远程 URL
  // 注意：run_kraken_ocr 后端可能只支持本地文件路径？
  // 如果后端支持 URL 下载，可以直接传 URL。如果不支持，需要确认逻辑。
  // 假设后端支持本地路径，如果只有 URL，你可能需要先下载。
  // 这里暂时假设 image_local_path 必须存在，或者 image_remote_url 指向的是本地可访问的资源。

  // 根据 Test.vue 的逻辑，imagePath 是必须的。
  const imagePath = meta.value?.image_local_path || meta.value?.image_remote_url;

  if (!imagePath) {
    ocrErrorMsg.value = '未找到有效的图片路径'
    return
  }

  // 检查模型选择
  if (!useDefaultRecModel.value && !ocrModelPath.value) {
    alert('请选择识别模型或使用默认模型')
    return
  }
  if (!useDefaultSegModel.value && !ocrSegModelPath.value) {
    alert('请选择分割模型或使用默认模型')
    return
  }

  ocrIsProcessing.value = true
  ocrErrorMsg.value = ''

  try {
    // 调用后端 OCR
    const resultXml = await invoke('run_kraken_ocr', {
      imagePath: imagePath,
      modelPath: useDefaultRecModel.value ? null : ocrModelPath.value,
      segModelPath: useDefaultSegModel.value ? null : ocrSegModelPath.value
    })

    // 处理结果：解析 XML 并更新当前页面数据
    // 1. 解析 XML 到 lines
    parseAltoXml(resultXml as string)

    // 2. 保存 ALTO 到数据库 (后端)
    altoXml.value = resultXml as string
    await invoke('save_alto', { pageId, xml: resultXml })

    // 3. 重置 Sentence 数据 (因为重新识别了，旧的句子结构失效)
    // 根据新的 TextLine 生成默认句子结构
    const newSentences: Sentence[] = lines.value.map(line => ({
      id: crypto.randomUUID(),
      line_ids: [line.id],
      type: 'text',
      normalized: '', // 这里可以从 XML 提取 CONTENT
      en: '',
      zh: ''
    }))
    ...
    // 4. 保存 Sentence 到数据库
    await invoke('save_sentence', { pageId, data: JSON.stringify(sentenceData.value) })

    // 5. 存历史记录
    saveHistory()

    // 6. 成功提示并关闭弹窗
    alert('OCR 识别成功！数据已更新。')
    ...
    ocrErrorMsg.value = `执行失败: ${e}`
    console.error(e)
  } finally {
    ocrIsProcessing.value = false
  }
}
...
// 鼠标按下：选择、开始拖拽、或新建点
// 鼠标按下：选择、开始拖拽、新建点、或开始平移画布
function handleMouseDown(e: MouseEvent) {
  // 获取相对于 Canvas 内容的逻辑坐标（用于点击检测）
  const { x, y } = getCanvasPos(e);

  // ----------------------------------------------------
  // 1. 补足基线模式 (Fix Missing Baseline) -> 优先级最高
  // ----------------------------------------------------
  if (targetMissingBaselineId.value) {
  ...
  // ----------------------------------------------------
  // 2. 新建多边形模式 (Create Polygon) -> 优先级次之
  // ----------------------------------------------------
  if (isCreating.value) {
    tempPoints.value.push([x, y]);
    drawCanvas();
  ...
  // 定义一个开始平移的辅助逻辑 (使用屏幕坐标 clientX/Y)
  const startPan = () => {
    isPanning.value = true;
    startPanPos.value = { x: e.clientX, y: e.clientY };
    startTranslate.value = { x: translateX.value, y: translateY.value };
    document.body.style.cursor = 'grabbing';
  };

  // 优先级 A: 如果显示多边形，优先检测多边形
  if (showPolygons.value) {
  ...
  // 优先级 C: 既没开多边形也没开基线（或者逻辑漏网之鱼），直接平移
  startPan();
  drawCanvas();
}
...
function handleMouseMove(e: MouseEvent) {
  ...
  if (isPanning.value) {
    ...
    translateX.value = startTranslate.value.x + dx;
    translateY.value = startTranslate.value.y + dy;
    return;
  }
...
function getCanvasPos(e: MouseEvent) {
  ...
  // 转换为 Canvas 内部坐标： (容器内坐标 - 偏移量) / 缩放
  return {
    x: (mouseXInContainer - translateX.value) / scale.value,
    y: (mouseYInContainer - translateY.value) / scale.value
  };
}
...
function handleWheel(e: WheelEvent) {
  ...
  // 1. 计算缩放前的鼠标在图片上的相对位置 (未缩放的像素单位)
  // 获取容器 Rect
  const container = (e.currentTarget as HTMLElement).getBoundingClientRect();
  const mouseX = e.clientX - container.left;
  const mouseY = e.clientY - container.top;

  // 计算鼠标点相对于当前视口原点的偏移 (即鼠标指向图片上的哪个点 * 当前缩放)
  const pointX = mouseX - translateX.value;
  const pointY = mouseY - translateY.value;

  // 2. 计算新缩放比例
  const zoomSensitivity = 0.001; // 滚轮灵敏度，可微调
  const delta = -e.deltaY * zoomSensitivity * scale.value; // 基于当前比例缩放，更平滑
  // 限制缩放范围 (例如 0.1 到 10)
  const newScale = Math.max(0.1, Math.min(10, scale.value + delta));

  // 3. 计算新的偏移量，保持鼠标指向的点位置不变
  // 公式：新偏移 = 鼠标位置 - (鼠标指向的点 * 新缩放 / 旧缩放)
  const scaleRatio = newScale / scale.value;

  translateX.value = mouseX - pointX * scaleRatio;
  translateY.value = mouseY - pointY * scaleRatio;
...
async function exportAltoXml() {
  if (!altoXml.value) {
    alert('没有可导出的 ALTO XML')
    return
  }

  const filePath = await save({
    filters: [{ name: 'ALTO XML', extensions: ['xml'] }],
    defaultPath: `page_${pageId}.xml`
  })
...
async function exportSentenceJson() {
  ...
  const filePath = await save({
    ...
    defaultPath: `page_${pageId}_sentences.json`
  })
...
<template>
  <div class="h-screen w-screen bg-zinc-950 text-zinc-100 flex flex-col overflow-hidden">
    <!-- 工具栏 -->
    <div class="h-12 bg-zinc-900 border-b border-zinc-700 flex items-center px-4 gap-2 shrink-0 z-20">
      ...
      <button class="px-3 py-1.5 rounded text-base font-medium bg-indigo-600 text-white hover:bg-indigo-500"
        @click="openOcrModal">
        ⚡识别
      </button>
      ...
      <button
        class="px-3 py-1.5 rounded text-sm font-medium bg-cyan-600 text-white hover:bg-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed"
        :disabled="!altoXml" @click="exportAltoXml">
        导出 XML
      </button>
      <button
        class="px-3 py-1.5 rounded text-sm font-medium bg-cyan-600 text-white hover:bg-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed"
        :disabled="!sentenceData" @click="exportSentenceJson">
        导出 JSON
      </button>
      <div class="w-px h-6 bg-zinc-700 mx-1"></div>
      <button :class="[
      ...
      <div class="flex items-center gap-3 text-base">
        <label class="flex items-center gap-1.5 cursor-pointer">
          ...
          <span class="text-zinc-400">原文</span>
        </label>
        <label class="flex items-center gap-1.5 cursor-pointer">
    ...
    <div class="flex-1 flex overflow-hidden relative">
      <!-- 左侧图片区 -->
      <div :class="[
        'bg-zinc-900 relative overflow-hidden transition-all duration-300',
        showTopology ? 'w-2/5' : 'w-1/2',
        ...
        <div v-if="isCreating || selectedLineId || targetMissingBaselineId"
          class="absolute top-12 left-2 flex flex-col gap-2 z-10" @mousedown.stop>
        ...
        <div v-if="imageUrl" class="origin-top-left will-change-transform" :style="{
          transform: `translate(${translateX}px, ${translateY}px) scale(${scale})`
        }">
          <img ref="imageRef" :src="imageUrl" class="max-w-none pointer-events-none select-none" @load="initCanvas" />

          <canvas ref="canvasRef" class="absolute top-0 left-0" @dblclick="handleDblClick"
      ...
      <!-- 右侧文本区 -->
      <div :class="[
        'bg-zinc-950 border-l border-zinc-800 p-4 overflow-y-auto transition-all duration-300 h-full',
        showTopology ? 'w-2/5' : 'w-1/2'
      ]">
            ...
            <div class="p-2 space-y-1">
              ...
              <div v-if="showNormalized || showEnglish || showChinese"
                ...
                <div v-if="showNormalized && sentence.normalized" class="text-base">
                  <span class="text-zinc-500 text-sm mr-2">原文:</span>
      ...
      <!-- 拓扑关系面板 -->
      <div v-if="showTopology" class="w-1/5 border-l border-zinc-800 transition-all duration-300">
...
<div v-if="showOcrModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
      @click.self="showOcrModal = false">
      <div class="bg-zinc-900 rounded-lg p-6 w-lg max-h-[90vh] overflow-y-auto border border-zinc-700 shadow-2xl">
        <h2 class="text-xl font-bold mb-6 flex items-center gap-2 text-zinc-200">
          <span>⚡</span> OCR 自动识别
        </h2>

        <div class="mb-6 p-3 bg-zinc-800/50 rounded border border-zinc-700/50">
          <div class="text-sm text-zinc-400 mb-1">当前处理图片：</div>
          <div class="text-zinc-200 text-sm truncate font-mono">
             {{ meta?.image_local_path || meta?.image_remote_url || '未知路径' }}
          </div>
        </div>
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-2">
            <h3 class="text-base font-medium text-zinc-300">识别模型 (Recognition Model)</h3>
            <div class="p-4 bg-zinc-800 rounded border border-zinc-700 space-y-3">
              <div class="flex items-center gap-2">
                <input type="checkbox" id="useDefaultRecModel" v-model="useDefaultRecModel"
                  class="w-4 h-4 accent-blue-600 cursor-pointer" />
                <label for="useDefaultRecModel" class="text-sm cursor-pointer select-none text-zinc-200">
                  使用默认模型 (catmus-medieval)
                  <span v-if="defaultModelsStatus.recognition_model" class="text-green-400 ml-2 text-xs">✓ 已就绪</span>
                  <span v-else class="text-red-400 ml-2 text-xs">✗ 未找到</span>
                </label>
              </div>
              
              <div v-if="!useDefaultRecModel" class="flex gap-2 pl-6 animate-in fade-in slide-in-from-top-1">
                <button class="px-3 py-1.5 bg-zinc-700 hover:bg-zinc-600 rounded text-sm text-zinc-200 shrink-0"
                  @click="selectOcrModel">
                  选择文件...
                </button>
                <div class="flex-1 px-3 py-1.5 bg-zinc-900 border border-zinc-700 rounded text-sm text-zinc-400 truncate">
                  {{ ocrModelPath || '请选择 .mlmodel 文件' }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-2">
            <h3 class="text-base font-medium text-zinc-300">分割模型 (Segmentation Model)</h3>
            <div class="p-4 bg-zinc-800 rounded border border-zinc-700 space-y-3">
              <div class="flex items-center gap-2">
                <input type="checkbox" id="useDefaultSegModel" v-model="useDefaultSegModel"
                  class="w-4 h-4 accent-blue-600 cursor-pointer" />
                <label for="useDefaultSegModel" class="text-sm cursor-pointer select-none text-zinc-200">
                  使用默认模型 (blla)
                  <span v-if="defaultModelsStatus.segmentation_model" class="text-green-400 ml-2 text-xs">✓ 已就绪</span>
                  <span v-else class="text-red-400 ml-2 text-xs">✗ 未找到</span>
                </label>
              </div>

              <div v-if="!useDefaultSegModel" class="flex gap-2 pl-6 animate-in fade-in slide-in-from-top-1">
                <button class="px-3 py-1.5 bg-zinc-700 hover:bg-zinc-600 rounded text-sm text-zinc-200 shrink-0"
                  @click="selectOcrSegModel">
                  选择文件...
                </button>
                <div class="flex-1 px-3 py-1.5 bg-zinc-900 border border-zinc-700 rounded text-sm text-zinc-400 truncate">
                  {{ ocrSegModelPath || '请选择 .mlmodel 文件' }}
                </div>
              </div>
            </div>
          </div>
        ...
        <div v-if="ocrErrorMsg" class="mt-6 p-3 bg-red-900/30 border border-red-800 rounded">
          <div class="flex items-start gap-2">
            <span class="text-red-400 mt-0.5">⚠️</span>
            <pre class="text-red-300 text-sm whitespace-pre-wrap font-sans">{{ ocrErrorMsg }}</pre>
          </div>
        </div>
        
        <div class="mt-6 p-3 bg-yellow-900/20 border border-yellow-800/50 rounded text-sm text-yellow-500/80">
          注意：运行 OCR 将会覆盖当前页面所有的 Line 和 Sentence 数据，此操作不可撤销（除非使用撤销功能）。
        </div>

        <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-zinc-800">
          <button class="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 rounded text-sm font-medium text-zinc-300"
            @click="showOcrModal = false" :disabled="ocrIsProcessing">
            取消
          </button>
          <button class="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 rounded text-sm font-medium text-white flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            @click="runOcr" :disabled="ocrIsProcessing">
            <span v-if="ocrIsProcessing" class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
            {{ ocrIsProcessing ? '正在处理...' : '开始识别' }}
          </button>
        </div>
      </div>
    </div>
